<template>
  <view class="doctor-top filter-view">
    <view class="all-text" :class="selectIndex==-1? 'changeColor' : ''"
    @click="allDoctor"
    >全部</view>
    ...
    <!-- <view class="doctor-list" @click="jumpRoute(d_id)">
        <view class="about-doctor">预约</view>
    </view> -->
    <!-- 提示组件 -->
    <point :show="doctor_list.length == 0 ? true : false" :title="title"></point>
</template>
...
let d_id = ref('1fe427876314d7ee000077bd1c17c91f')

// 存储选择日期
let doctor_time = ref<Doctortime[]>([])
// 存储医生列表
let doctor_list = ref<Doctorlist[]>([])
let dep_id = ref('')
// 请求数据
interface ResData{
    data:{data:any[]}
...
async function selectDoctor(index:number,dep_id:string,week:string){
    ...
    const res = await RequestApi.EverydList({dep_id:dep_id,week}) as ResData
    doctor_list.value = res.data.data
}
// 跳转医生主页
